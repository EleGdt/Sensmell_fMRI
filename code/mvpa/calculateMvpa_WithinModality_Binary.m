function accu = calculateMvpa_WithinModality_Binary(opt)
    ...
    %% MVPA options
    decodingConditionList = {'olfaWords_vs_nonOlfaWords','olfaPictures_vs_nonOlfaPictures'};
...
%     opt.maskLabel = {'lACC', 'rACC', 'lInsula', 'rInsula', 'lOFCmed', 'rOFCmed'};
    ...
    %get the masks/ROIs to be used
    opt.maskPath = fullfile(fileparts(mfilename('fullpath')), '..','..', 'outputs','ROI_Resliced');
        ...
        for iImage = 1:length(opt.mvpa.map4D) %set 4D image
            ...
            for iMask = 1:length(opt.maskLabel)
                ...
                for iDecodingCondition = 1:length(decodingConditionList)
                    ...
                    condLabelNb = [1 2 3 4]; % ici 1 to 4 (4 conditions)
                    condLabelName = {'olfaWords', 'nonOlfaWords', 'olfaPictures', 'nonOlfaPictures'}; % vérifier que l'ordre est correct!!
